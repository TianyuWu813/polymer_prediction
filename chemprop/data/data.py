...
def bigsmilestosmiles(bigsmiles):
    ...
    for i in range(len(repeat_scaffold_origin)):
        ...
        ratio_scaffold.append((repeat_scaffold_origin[i].split('.'))[1])
    ...
    return forward_smile, repeat_scaffold, end_scaffold,ratio_scaffold  #, backward_smile
...
class MoleculeDatapoint:
    ...
    def __init__(self,
        ...
        self.bigsmiles = line[0]
        self.smiles = line[0]  # str   重要
        # #######
        self.forward_smile_mor, self.repeat_scaffold_mor, self.end_scaffold_mor,self.ratio_scaffold_mor = bigsmilestosmiles(self.smiles)
        self.repeat_mol_list_mor =[]  #重复单元的 smile形式
        self.ratio_list_mor = []
        for i in range(len(self.repeat_scaffold_mor)):
            self.repeat_mol_list_mor.append(bond_clear(self.repeat_scaffold_mor[i]))
            self.ratio_list_mor.append(ratio_clear(self.ratio_scaffold_mor[i]))



        self.mol_mor = Chem.MolFromSmiles(self.repeat_mol_list_mor[0])  # 重要
        self.mol2_mor = Chem.MolFromSmiles(self.repeat_mol_list_mor[1])  # 重要
        self.ratio1_mor=self.ratio_list_mor[0]
        self.ratio2_mor = self.ratio_list_mor[1]
        ######
        self.mol = Chem.MolFromSmiles(self.smiles)  #重要
        ...
        if self.features_generator is not None:
            ...
            for fg in self.features_generator:
                ...
                if self.mol_mor is not None and self.mol_mor.GetNumHeavyAtoms() > 0:
                    feature_1 = features_generator(self.mol_mor)
                    feature_2 = features_generator(self.mol2_mor)
                    # print(feature_1,feature_2)
                    feature_1 = np.array(feature_1).tolist()
                    feature_1.append(float(self.ratio1_mor))

                    feature_2 = np.array(feature_2).tolist()
                    feature_2.append(float(self.ratio2_mor))

                    features_all = feature_1+feature_2
                    self.features.extend(features_all)
