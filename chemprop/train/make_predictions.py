...
from .predict import predict,predict_feature
from chemprop.data import MoleculeDataset
from chemprop.data.utils import get_data, get_data_from_smiles
from chemprop.utils import load_args, load_checkpoint, load_scalers


def make_predictions(args: Namespace, smiles: List[str] = None, features=None) -> List[Optional[List[float]]]:
    """
    ...
    valid_indices = [i for i in range(len(test_data)) if test_data[i].mol is None]
    # valid_indices = [i for i in range(len(test_data)) if test_data[i].mol is not None]  for 小分子
    full_data = test_data
    ...
    for checkpoint_path in tqdm(args.checkpoint_paths, total=len(args.checkpoint_paths)):
        ...
        model_preds,all_feature = predict_feature(
            model=model,
            data=test_data,
    ...
    return avg_preds, test_data.smiles(),all_feature

def make_predictions_morgan(args: Namespace, smiles: List[str] = None, features=None) -> List[Optional[List[float]]]:
    """
    Makes predictions. If smiles is provided, makes predictions on smiles. Otherwise makes predictions on args.test_data.
